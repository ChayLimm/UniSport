...
class CheckpointProvider extends ChangeNotifier {
  ...
  Future<bool> recordCheckpoint({
    ...
    final data = await CheckpointService.instance.recordCheckpoint(checkpoint);
    print(data);
  ...
  DateTime getParticipantCheckpointTime(int segmentId, int participantId) {
    final checkpoints = _checkpoints;//CheckpointService.instance.getCheckpointsBySegmentId(segmentId);
  ...
  Future<void> getCheckpointsBySegmentId(int segmentID)async{
    final data = await CheckpointService.instance.fetchCheckpointBySegmentId(segmentID);
    _checkpoints.clear();
  ...
  bool isCompleted(int segmentId, int participantId) {
    ...
    for (var checkpoint in checkpoints) {
      if (checkpoint.participantId == participantId && checkpoint.segmentId == segmentId) {
        return true;
      }
    }
      return false;
